...
class Sprite(object):
    ...
    def __init__(self, filename, x, y, fps=FU_FRAME_RATE):
        ...
        self.current_frame = self.anims[self.current_anim][self.current_frame_num]
    ...
    def next_frame(self):
        self.lastdt = datetime.now()
        self.current_frame_num = (self.current_frame_num + 1) % len(self.anims[self.current_anim])
        self.current_frame = self.anims[self.current_anim][self.current_frame_num]
